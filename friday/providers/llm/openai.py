...
class OpenAiLlmProvider(LlmProvider):
    ...
    async def health_check(self) -> bool:
        try:
            # Quick health check calling low-cost model or checking auth
            if not self.api_key:
                return False
            return True
        except Exception:
    ...
    async def chat_stream(self, messages: List[LLMMessage], **kwargs) -> AsyncIterator[str]:
        # Simple non-blocking generator wrapper for streaming simulation or real API stream
        # To support real stream, we can implement it cleanly.
        url = "https://api.openai.com/v1/chat/completions"
        headers = {
            "Authorization": f"Bearer {self.api_key}",
            "Content-Type": "application/json"
